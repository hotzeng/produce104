...
/* A list of tasks which are ready to run. */
node_t ready_queue;

/* The sum of the priorities of
 * all tasks in the ready queue
 */
priority_t total_ready_priority;


/* An ORDERED queue of processes
 * which are sleeping.
 * They are ordered by pcb->sleep_until, ascending.
 * Said another way, the first element of this
 * queue is the first which will wake up.
 */
node_t sleep_queue;

/* System idle "task"
 * Not a task per se, but does something
 * to kill time when everything else is sleeping
 */
static void idle(void);

/* Order sleeping tasks, so that they are ordered
 * by their wake-up times.  Tasks which are scheduled
 * to wake up earlier will appear earlier in the list.
 */
...
static void try_wake()
{
  ...
  /* Extract as many PCBs from the sleep queue
   * as are ready to be awoken.
   * Recall that the sleep queue is sorted
   * by wake-up times, ascending
   */
  while( !queue_empty(&sleep_queue) )
  {
    /* Inspect (but do not remove)
     * the first process to be woken up
     */
    pcb_t *sleeper = (pcb_t*) queue_first(&sleep_queue);

/*
    printf(0,20,"I will wake pid %u at %u <=> %d    ",
      sleeper->pid,
      (uint32_t)sleeper->sleep_until,
      (uint32_t)now);
*/

    if( now >= sleeper->sleep_until )
    {
      /* We should wake this thread */
      ...
      /* Next, consider other PCBs in the sleep queue */
      continue;
    }
    else
    {
      /* If the first doesn't wake up yet,
       * then none of the others could
 ...
 * If all processes are sleeping, this
 * kills time...
 */
static void idle(void)
{
  ...
  /* you may put things here if you wish */
}
